...
To run NVTabular on the Databricksm, do the following:

1. First we need to create a custome NVTabular container based on Databricks runtime. 

2. Databricks is very sensitive to it dependecy which comes natively with the Databricks ccontainer runtime. 

3. The Nvidia RAPIDS team has a already put together a [Docker container](https://github.com/rapidsai/cloud-ml-examples/tree/main/databricks/docker) to run RAPIDS inside a databricks cluster.
...
9. Once the contianer is pushed to a repo. we can now use the custom container to spin up our databricks cluster.
