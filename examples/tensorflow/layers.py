...
class ScalarDenseFeatures(tf.keras.layers.Layer):
    ...
    def __init__(self, feature_columns, aggregation="concat", name=None, **kwargs):
        ...
        # sort feature columns to make layer independent of column
        # feed order
        ...
        for feature_column in feature_columns:
            if isinstance(feature_column, fc.CategoricalColumn):
                ...
                # for embedding, record both dimensions, otherwise only need
                # one dim for indicator
                if isinstance(feature_column, fc.EmbeddingColumn):
                    embedding_shapes[feature_name] = (
                        num_buckets,
                        feature_column.dimension,
                    )
                else:
                    embedding_shapes[feature_name] = (num_buckets,)

            elif isinstance(feature_column, fc.NumericColumn):
                if aggregation == "stack":
                    raise ValueError(
                        "Stacked aggregation not compatible with numeric columns"
                    )
                _validate_numeric_column(feature_column)
                numeric_features.append(feature_column.key)

        assert aggregation in ("concat", "stack")
        if aggregation == "stack":
            embedding_dims = [i[-1] for i in embedding_shapes.values()]
            embedding_dim = embedding_dims[0]
            assert all([i == embedding_dim for i in embedding_dims[1:]])
    ...
    def build(self, input_shapes):
        ...
        for feature_name, embedding_shape in self.embedding_shapes.items():
            if len(embedding_shape) == 2:
                ...
                self.embedding_tables[feature_name] = self.add_weight(
                    name="{}/embedding_weights".format(feature_name),
                    trainable=False,
                    initializer=tf.constant_initializer(np.eye(embedding_shape[0])),
                    shape=(embedding_shape[0], embedding_shape[0]),
                )
        self.built = True

    def call(self, inputs):
        dense_features = [
            inputs[feature_name] for feature_name in self.numeric_features
        ]

        categorical_feature_names = sorted(self.embedding_shapes.keys())
        for feature_name in categorical_feature_names:
            table = self.embedding_tables[feature_name]
            embeddings = tf.gather(table, inputs[feature_name][:, 0])
            dense_features.append(embeddings)

        if self.aggregation == "stack":
            return tf.stack(dense_features, axis=1)
