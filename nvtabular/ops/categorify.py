...
class Categorify(StatOperator):
    ...
    def fit(self, columns: Columns, ddf: dd.DataFrame):
        ...
        columns_uniq = columns.names().flatten().unique()
        columns_all = columns.names().flatten()
        ...
        for group in columns.names().nesting_to_tuple():
    ...
    def transform(self, columns: Columns, df: DataFrameType) -> DataFrameType:
        ...
        if self.encode_type == "combo":
            ...
            cat_names, multi_col_group = _get_multicolumn_names(
                columns.names().to_list(), df.columns, self.name_sep
            )
        else:
            # Case (1) & (2) - Simple 1-to-1 mapping
            multi_col_group = {}
            cat_names = columns.names().flatten()
