...
class PowerLawDistro():
    ...
    def create_col(self, num_rows, dtype=np.float32, min_val=0, max_val=1):
        ...
        factor = (cupy.power(max_val, gamma) - cupy.power(min_val, gamma)) + cupy.power(min_val, gamma)
        ser = ser * factor.item()
        exp = 1.0/gamma
        ser = ser.pow(exp)
...
class DatasetGen():
    ...
    def create_cats(self, size, num_cols=1, cardinality=[1]):
        ...
        df = cudf.DataFrame()
        for x in range(num_cols):
            ser = self.dist.create_col(size, dtype=np.long, min_val=1.0, max_val=cardinality[x]).ceil()
            ser.name = f"CAT_{x}"
            df = cudf.concat([df, ser], axis=1)
        return df
