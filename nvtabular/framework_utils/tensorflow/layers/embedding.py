...
class SparseEmbeddingLookup(tf.keras.layers.Layer):
    """
    Layer which enables the use of multi-hot categorical features.
    It feeds each category in a Embedding Layer and take the sum of
    multi-hot embeddings. It is implemented as SparseEmbeddingLookup.

    Example usage::

        _MAX_NNZS = [20, 100]
        _SPARSE_DIMS = [1000, 10000]
        _BATCH_SIZE = 128

        RaggedLayer = tf.keras.layers.Lambda(
            lambda x: tf.RaggedTensor.from_row_lengths(
                x[0][:, 0], x[1][:, 0]).to_sparse()
        )

        xs, inputs = [], []
        for n, (max_nnz, sparse_dim) in enumerate(zip(_MAX_NNZS, _SPARSE_DIMS)):
            values = tf.keras.Input(name=f"values_{n}", dtype=tf.int64, shape=(1,))
            row_lengths = tf.keras.Input(name=f"row_lengths_{n}", dtype=tf.int64, shape=(1,))
            inputs.extend([values, row_lengths])

            x = RaggedLayer([values, row_lengths])
            xs.append(SparseEmbeddingLookup(sparse_dim, 8)(x))
        ...
        model.fit(get_data(_BATCH_SIZE), epochs=1, steps_per_epoch=100)

    Parameters
    ----------
    vocab_size : int
        vocabulary Size of input category
    dimension : int
        output dimension of Embedding layer
    """

    def __init__(self, vocab_size, dimension, **kwargs):
        self.vocab_size = vocab_size
        self.dimension = dimension
        super(SparseEmbeddingLookup, self).__init__(**kwargs)
    ...
    def call(self, x):
        return tf.nn.embedding_lookup_sparse(self.embedding_table, x, None, combiner="sum")
