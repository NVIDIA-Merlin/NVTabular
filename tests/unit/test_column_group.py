...
def test_column_group_select():
    ...
    sqrt_features = input_features[["a", "c"]] >> (lambda col: col ** (1.0 / 2.0))
    plus_one_features = input_features["b"] >> (lambda col: col + 1)
    features = sqrt_features + plus_one_features

    workflow = Workflow(features)
    df_out = (
        workflow.fit_transform(Dataset(df, cpu=not cudf)).to_ddf().compute(scheduler="synchronous")
    )

    expected = _lib.DataFrame()
    expected["a"] = df["a"].pow(1.0 / 2.0)
    expected["c"] = df["c"].pow(1.0 / 2.0)
    expected["b"] = df["b"] + 1
